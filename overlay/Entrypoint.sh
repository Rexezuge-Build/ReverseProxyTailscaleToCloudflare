...
sleep 5
TS_AUTHKEY=$(curl -X GET "$PROVIDER_BASE_URL/tailscale" -H "Authorization: Bearer $BEARER_TOKEN")
CLOUDFLARE_TOKEN=$(curl -X GET "$PROVIDER_BASE_URL/cloudflare" -H "Authorization: Bearer $BEARER_TOKEN")
...
/usr/local/bin/cloudflared tunnel --no-autoupdate --loglevel error run --token "$CLOUDFLARE_TOKEN" > /dev/null 2> /dev/null
