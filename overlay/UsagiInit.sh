...
/usr/sbin/tailscaled -no-logs-no-support --tun=userspace-networking --socks5-server=localhost:1055 --disable-udp
sleep 10s
/usr/bin/tailscale up --auth-key $TS_AUTHKEY --accept-dns=false --accept-routes=false --ssh
